...
const testController = {
    addQuestionnaires: async (req, res) => {
        try {
            ...
            if(!name) {
                return res.status(200).json({message: 'A questionnaire name must be provided'})
            }

            // check if questionnaire already exists
            const checkQuestionnaire = await Questionnaires.findOne({name})
            if(checkQuestionnaire) {
                return res.status(200).json({message: 'Questionnaire already exists'})
            }
    ...
    getQuestionnaires: async (req, res) => {
        try {
            ...
            if(checkRelationUserQuestionnaire.length == 0) {
                return res.status(200).json({message: 'User does not have questionnaires assigned'})
            }
    ...
    addQuestionnaireToUser: async (req, res) => {
        try {
            ...
            if(!checkQuestionnaire) {
                return res.status(200).json({message: 'Questionnaire does not exist'})
            }
            ...
            checkRelationUserQuestionnaire.forEach( (obj) => { 
                if(obj.questionnaireId == questionnaireId) {
                    return res.status(200).json({message: 'Questionnaire already assigned to user'})
                }
            })
    ...
    getQuestion: async(req, res) => {
        ...
        try {
            ...
            if(!checkQuestionnaire) {
                return res.status(200).json({message: 'Questionnaire does not exists'})
            }
            ...
            const checkRelationUserQuestionnaire = await RelationUsersQuestionnaires.find({userId})
            if(checkRelationUserQuestionnaire.length == 0) {
                res.status(200).json({message: 'User does not have questionnaires assigned'})
            }
                    ...
                    questionUserIdRelation.forEach( async (questionId) => {
                        if (questionId != obj._id) {
                            ...
                            if(checkOptions.length == 0) {
                                return res.status(200).json({message: 'There are no options available for this question'})
                            }
    ...
    updateAnswer: async (req, res) => {
        try {
            ...
            if(!checkOptions) {
                return res.status(200).json({message: 'Option provided does not exist'})
            }
            ...
            if(!checkUserAnswerRelation) {
                return res.status(200).json({message: 'Answer does not match with user records'})
            }
